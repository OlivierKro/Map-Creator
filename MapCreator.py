...
def menu_mapy():
    ...
    def stworz_mape(okno_stare):
        ...
        def zapisz_mape_do_csv(nazwa_pliku):
            ...
            id_elementow_mapy = okno_mapa.find_all()
            zapis.write(f"{'Id'}; {'Tag'}; {'Typ'}; {'Kolor'}; {'Współrzędne'}\n")
            for i in id_elementow_mapy:
                ...
                zapis.write(f'{Id}; {Tag[0]}; {Typ}; {Kolor}; {Cords}\n')
            zapis.close()
        ...
        def proces_zapisania(okno_zapisu, okno_aktualne, wpisz_nazwa_pliku):
            ...
            Funkcja nie zwraca żadnej wartości
            '''
            if os.path.exists("mapa w ppm") == False:
                os.makedirs("mapa w ppm")
            if os.path.exists("mapa w png") == False:
                os.makedirs("mapa w png")
            if os.path.exists("mapa w csv") == False:
                os.makedirs("mapa w csv")
            nazwa_pliku = wpisz_nazwa_pliku.get()
            wpisz_nazwa_pliku.delete(0, 'end')
            nazwa_ppm = 'mapa w ppm' + '\\' + nazwa_pliku + '.ppm'
            nazwa_png = 'mapa w png' + '\\' + nazwa_pliku + '.png'
            nazwa_csv = 'mapa w csv' + '\\' + nazwa_pliku + '.csv'
    ...
    def usun_mape(okno_stare):
        ...
        def podglad_mapy():
            ...
            if zbior_dostepnych_map.curselection() != ():
                ...
                sciezka_ppm = 'mapa w ppm' + '\\' + nazwa_ppm
                nazwa_png = nazwa_ppm.replace(".ppm", ".png")
                sciezka_png = 'mapa w png' + '\\' + nazwa_png
    ...
    przycisk_wyjscie = tk.Button(okno_menu_mapy, text="Zamknij aplikację", width='30', height='3', font = ("timesnewroman", 15, 'bold'), command=lambda: wyjscie())
    ...
    przycisk_wyjscie.pack()
    przycisk_wyjscie.place(x=SZEROKOSC_OKNA/2, y=WYSOKOSC_OKNA/2 + 100, anchor="center")
